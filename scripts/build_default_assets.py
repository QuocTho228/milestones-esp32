...
def get_text_font_path(builtin_text_font, milestones_fonts_path):
    """
    ...
    font_path = os.path.join(milestones_fonts_path, 'cbin', font_name)
...
def get_emoji_collection_path(default_emoji_collection, milestones_fonts_path, project_root=None):
    """
    Get the emoji collection path if needed
    Returns the emoji directory path or None if no emoji collection is needed
    
    Supports:
    - PNG emoji collections from milestones-fonts (e.g., emojis_32, twemoji_64)
    - GIF emoji collections from milestones-fonts (e.g., noto-emoji_128, noto-emoji_64)
    ...
    emoji_path = os.path.join(milestones_fonts_path, 'png', default_emoji_collection)
    if os.path.exists(emoji_path):
        return emoji_path
    
    # Try GIF emoji collections (e.g., noto-emoji_128, noto-emoji_64, noto-emoji_32)
    emoji_path = os.path.join(milestones_fonts_path, 'gif', default_emoji_collection)
    if os.path.exists(emoji_path):
        return emoji_path
...
def main():
    ...
    parser.add_argument('--milestones_fonts_path', help='Path to milestones-fonts component directory')
    parser.add_argument('--extra_files', help='Path to extra files directory to be included in assets')
    
    args = parser.parse_args()
    
    # Set default paths if not provided
    if not args.esp_sr_model_path or not args.milestones_fonts_path:
        ...
        if not args.milestones_fonts_path:
            args.milestones_fonts_path = os.path.join(project_root, "components", "milestones-fonts")
    ...
    text_font_path = get_text_font_path(args.builtin_text_font, args.milestones_fonts_path)
    
    # Get emoji collection path if needed
    # Calculate project root from script location for otto-gif support
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(script_dir)
    emoji_collection_path = get_emoji_collection_path(args.emoji_collection, args.milestones_fonts_path, project_root)
