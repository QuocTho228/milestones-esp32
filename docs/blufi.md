...
This document explains how to enable and use BluFi (BLE-based WiFi provisioning) in the Milestones firmware, together with the in-tree `esp-wifi-connect` component that handles WiFi connection and credential storage. See the official [Espressif BluFi documentation](https://docs.espressif.com/projects/esp-idf/en/stable/esp32/api-guides/ble/blufi.html) for the protocol details.
...
- Because the BluFi API changed in IDF 5.5.2, firmware built with 5.5.2 advertises the Bluetooth name as `"Milestones-Blufi"`, while 5.5.1 uses `"BLUFI_DEVICE"`.
