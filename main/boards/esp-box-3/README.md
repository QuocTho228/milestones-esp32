...
- `Milestones Assistant` → `Board Type` → 选择 `ESP BOX 3`

### UI风格选择

ESP-BOX-3 支持多种不同的 UI 显示风格，通过 menuconfig 配置选择：

- `Milestones Assistant` → `Select display style` → 选择显示风格
...
1. `Milestones Assistant` → `Flash Assets` → 选择 `Flash Custom Assets`
2. `Milestones Assistant` → `Custom Assets File` → 填入资源文件地址：
   ```
   https://dl.espressif.com/AE/wn9_nihaomilestones_tts-font_puhui_common_20_4-esp-box-3.bin
   ```
...
- `Milestones Assistant` → `Enable Device-Side AEC` → 启用
...
- `Milestones Assistant` → `Wake Word Implementation Type` → 选择唤醒词类型
